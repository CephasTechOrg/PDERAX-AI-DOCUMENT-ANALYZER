...
class PDERAXApp {
    ...
    async checkForHistoryAnalysis() {
        ...
        this.showLoading();
        try {
            const result = await window.apiService.getAnalysisById(analysisId);
            const displayData = {
            ...
            this.currentAnalysis = displayData;
            this.displayResults(displayData);
        } catch (e) {
            this.showUpload();
            this.showToast('Could not load analysis: ' + e.message, 'error');
        }
    }
