...
// Load a saved quiz set from history by ID
async function checkForQuizHistory() {
    const urlParams = new URLSearchParams(window.location.search);
    const quizSetId = urlParams.get('quiz_set_id');
    if (!quizSetId) return;
    try {
        const set = await window.apiService.getQuizSetById(quizSetId);
        quizQuestions = set.questions;
        showQuizTaking();
        showQuizToast(`Loaded "${set.title || 'Quiz'}" from history`, 'success');
    } catch (e) {
        showQuizToast('Could not load quiz: ' + e.message, 'error');
    }
}
