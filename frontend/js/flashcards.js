...
function checkForDocumentContext() {
    ...
    // Load flashcard set directly from history
    const flashcardSetId = urlParams.get('flashcard_set_id');
    if (flashcardSetId) {
        loadFlashcardSetFromHistory(flashcardSetId);
        return;
    }
...
// Load a saved flashcard set from history by ID
async function loadFlashcardSetFromHistory(id) {
    try {
        const set = await window.apiService.getFlashcardSetById(id);
        flashcards = set.cards;
        showResults();
        showToast(`Loaded "${set.title || 'Flashcard Set'}" from history`, 'success');
    } catch (e) {
        showToast('Could not load flashcard set: ' + e.message, 'error');
    }
}
        ...
        navActions.innerHTML = `
            <div class="user-menu">
                ...
                <div class="user-dropdown" id="userDropdown">
                    <a href="analyzer.html" class="dropdown-item">
